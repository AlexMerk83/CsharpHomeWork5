...
{
    Console.Clear();
    System.Console.WriteLine("A new array will be generated filled with random 3-digit integer numbers.");
    ...
    System.Console.WriteLine($"Quantity of even numbers in the array is {evenNumCount}");
}
...
{
    Console.Clear();
    System.Console.WriteLine("Enter parameters of a random array.");

    int[] array = ArrayHandler.GetRandomIntArray(ConsoleIOHandler.ReadInt("the array length"),
    ...
    System.Console.WriteLine($"Sum of the numbers in the odd positions is {evenElementsSum}");
}
...
class ConsoleIOHandler
{
    ...
    public const string defaultErrorMessage = "Input error. Please enter an integer number.";
    ...
    public static int ReadInt(string argument, int minValue, int maxValue, string errorMessage = defaultRangeErrorMessage)
    {
        ...
        inputFieldX = Console.CursorLeft;
        inputFieldY = Console.CursorTop;
        while (!int.TryParse(Console.ReadLine(), out intNum) || intNum < minValue || intNum > maxValue)
        {
            Console.SetCursorPosition(0, inputFieldY);
            ClearLine();
            System.Console.WriteLine($"Enter {argument}: ");
